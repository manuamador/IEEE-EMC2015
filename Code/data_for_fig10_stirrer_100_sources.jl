...
#n_pos=10
#x_p=linspace(0.5,a-0.5,n_pos)#rand(n_pos)*(a-2)+1
#y_p=linspace(0.5,b-0.5,n_pos)#rand(n_pos)*(b-2)+1
#z_p=linspace(0.5,d-0.5,n_pos)#rand(n_pos)*(d-2)+1
...
for u=1:nf
  ...
  savefig("N_1stirrer100source$u.png")
  clf()
  M=[]
  N=[]
  P=[]
...
using NPZ
npzwrite("Exstirrer100sources.npz", ["Ex" => Ex, "f" => f, "Q" => Q, "x" => x,"y" => y,"z" => z,"Nx"=>Nx,"Nmodesx"=>Nmodesx])

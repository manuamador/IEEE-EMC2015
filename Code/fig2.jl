...
for u=1:nf
  n_x=int(f[u]*(1+300/Q[u])*2*a/c)
  n_y=int(f[u]*(1+300/Q[u])*2*b/c)
  n_z=int(f[u]*(1+300/Q[u])*2*d/c)
  M=[]
  N=[]
  P=[]
  F=[]
...
figure(num=2,figsize=(8,10))
subplot(311)
title("\$ \\vert E_x\\vert\$")
loglog(f,abs(Ex))#,label="\$ \\vert E_x\\vert\$")
ylabel("A.U")
xlim(f0,f1)
grid()
subplot(312)
title("\$ \\vert E_y\\vert\$")
loglog(f,abs(Ey))#,label="\$ \\vert E_x\\vert\$")
ylabel("A.U")
xlim(f0,f1)
grid()
subplot(313)
title("\$ \\vert E_z\\vert\$")
loglog(f,abs(Ez))#,label="\$ \\vert E_x\\vert\$")
ylabel("A.U")
xlim(f0,f1)
grid()
xlabel("\$f\$ in Hz")

...
const df=2e6
const nf=1
const f=[300e6]#[224e6]#[208e6]#[382e6]#linspace(f0,f1,nf)#array([20e6,40e6,45e6,56e6,65e6])#linspace(20e6,.3e9,281)#linspace(20e6,3e8,1121)
const V=a*b*d
...
for i=1:n_sources
  phi0=rand()*2pi
  r0=rand()
  x_b[i,:]=ones(n_steps)*(i*(a-0.05)/n_sources+0.025)
  y_b[i,:]=1.5+r0*sin(phi+phi0)
  z_b[i,:]=3.5+r0*cos(phi+phi0)
end
...
cmax_r=maximum(abs(real(Ex[u,:,:,:])))

using PyPlot

for o=1:n_steps
  figure(num=0,figsize=(28.64,14.9))
  ftext=round(f[u]/1e6*100)/100
  phitext=round(phi[o]*180/pi)
  title("\$\\vert E_x\\vert\$, in plane \$x=1\$ m, \$ \\phi= $phitext^{\\circ}\$",fontsize=25)
  pcolor(z,y,squeeze(squeeze(real(Ex[u,o,:,:])/cmax_r,1),1),cmap="RdBu_r")
  plot(z_b[:,o],y_b[:,o],linestyle="None",marker="o",markerfacecolor="w", markersize=10)
  #clim(-1.5,1.5)
  colorbar(format="%.2f",orientation="vertical")
  xlabel("\$ z\$/m",fontsize=15)
  ylabel("\$ y\$/m",fontsize=15)
  axis("image")
  grid()
  savefig("Ex_$o.png",format="png",bbox_inches="tight",dpi=100)# , transparent="True")
  clf()
end


for o=1:n_steps
  figure(num=1,figsize=(24.64,14.9))
  ftext=round(f[u]/1e6*100)/100
  phitext=round(phi[o]*180/pi)
  title("\$ E_x \$, in plane \$x=1\$ m, \$ \\phi= $phitext^{\\circ}\$",fontsize=25)
  plot3D(z_b[:,o],y_b[:,o],x_b[:,o],lw=2,marker="o",markerfacecolor="w", markersize=10)
  plot([0,d], [0,b], [0,a], linestyle="None","w")
  surf(z,y,squeeze(squeeze(real(Ex[u,o,:,:])/cmax_r/1.1,1),1)+1,rstride=1, cstride=1, cmap="RdBu_r",linewidth=0)
  xlabel("\$ z\$/m",fontsize=20)
  ylabel("\$ y\$/m",fontsize=20)
  zlabel("\$ x\$/m",fontsize=20)
  #axis("equal")
  axis("image")
  grid()
  savefig("Ex3D_$o.png",format="png",bbox_inches="tight",dpi=100)# , transparent="True")
  clf()
end

...
using Grid
x_pi=InterpGrid(x_p,BCperiodic,InterpQuadratic)
y_pi=InterpGrid(y_p,BCperiodic,InterpQuadratic)
z_pi=InterpGrid(z_p,BCperiodic,InterpQuadratic)
...
ylabel("\$y\$/m")
zlabel("\$z\$/m")
savefig("movementb.pdf",bbox_inches="tight")
close()
...
for u=1:nf
  ...
  figure(num=0,figsize=(10,10))
  imshow(abs(ax))
  title("\$\\vert r_{x}\\vert \$, \$f=$ftext\$ MHz, \$ N_{x_{eff}}=$Nxtxt \$ pos.",fontsize=15)
  xlabel("Source steps")
  ylabel("Source steps")
  colorbar(shrink=0.7)
  clim(0,1)
  grid()
  savefig("mat_$u.png",bbox_inches="tight")
  clf()
...
using NPZ
npzwrite("Ex1sbrownian.npz", ["Ex" => Ex, "f" => f, "Q" => Q, "x_b" => x_b,"y_b" => y_b,"z_b" => z_b,"x" => x,"y" => y,"z" => z,"Nx"=>Nx,"Nmodesx"=>Nmodesx])
